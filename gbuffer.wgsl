...
@group(0) @binding(1) var<storage, read> materials: array<vec4f>; // 10 vec4f per material
...
@fragment fn fs(in: VsOut, @builtin(front_facing) front: bool) -> FsOut {
  ...
  // Material data layout: 10 vec4f per mat
  ...
  let m2 = materials[mi * 10u + 2u];
  let m3 = materials[mi * 10u + 3u];
  let m4 = materials[mi * 10u + 4u];
  let m5 = materials[mi * 10u + 5u];
  ...
  if alpha_mode >= 1u {
    let uv = select(in.uv0, in.uv1, m5.y >= 0.5);
    var alpha = m5.x;
    if base_tex >= 0 {
  ...
  let debug_uv = select(in.uv0, in.uv1, m5.y >= 0.5);
